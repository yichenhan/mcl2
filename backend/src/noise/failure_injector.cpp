...
void FailureInjector::apply(
    ...
    double& heading_deg) {
    // Delegate to split implementations for backward compatibility.
    applyOdom(tick, odom_delta, heading_deg);
    applySensors(tick, readings);
}

void FailureInjector::applyOdom(int tick, sim::MotionDelta& odom_delta, double& heading_deg) {
...
void FailureInjector::applySensors(int tick, std::array<double, 4>& readings) {
    for (const auto& e : events_) {
        if (!is_active(e, tick)) continue;
        switch (e.type) {
    ...
    // Reset stuck state for sensors no longer in a SensorStuck window.
