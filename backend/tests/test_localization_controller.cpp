...
TEST_CASE("LocalizationController computes odom deltas from absolute pose") {
    ...
    cfg.gate_enables = {false, false, false, false};
    mcl::LocalizationController c(cfg);
...
TEST_CASE("LocalizationController applies centroid jump gate") {
    ...
    CHECK(out.gate.failed_centroid_jump);
}

TEST_CASE("LocalizationController rejects large jumps when centroid jump gate enabled") {
    mcl::ControllerConfig cfg;
    cfg.algorithm = mcl::LocAlgorithm::MCL;
    cfg.mcl_config.num_particles = 80;
    cfg.min_sensors_for_update = 5; // skip update path for determinism
    cfg.gate_config.max_centroid_jump_ft_per_s = 0.5;
    cfg.gate_config.max_jump_in = 100.0;
    cfg.gate_enables = {true, false, false, false};
    mcl::LocalizationController c(cfg);
...
TEST_CASE("LocalizationController MCL populates predicted readings and residuals") {
    mcl::ControllerConfig cfg;
    cfg.algorithm = mcl::LocAlgorithm::MCL;
    cfg.mcl_config.num_particles = 120;
    cfg.min_sensors_for_update = 1;
    mcl::LocalizationController c(cfg);
    ...
    for (size_t i = 0; i < out.mcl_predicted_readings.size(); ++i) {
        if (out.mcl_predicted_readings[i] >= 0.0) any_predicted = true;
        if (out.mcl_sensor_residuals[i] > 0.0) any_residual = true;
    }
