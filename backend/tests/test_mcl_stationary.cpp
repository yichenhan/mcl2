...
TEST_CASE("MCL scoring: cap boundary, error at exactly max is inlier") {
    ...
    MCLConfig cfg;
    cfg.num_particles = 2;
    // Force a flat close-range outlier cap so "at cap" vs "over cap" is unambiguous.
    cfg.outlier_close_range_in = 1e9;
    cfg.outlier_close_cap_in = 2.0;
    ...
    const double err_cap = cfg.outlier_close_cap_in;
    at_cap[LEFT] += err_cap;
    over_cap[LEFT] += err_cap + 0.01;
