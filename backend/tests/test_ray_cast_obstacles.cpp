...
TEST_CASE("ray obstacle: empty obstacle list equals wall-only ray") {
    ...
    const double d_obs = ray::ray_distance_with_obstacles(pos, 0.0, offset, 0.0, std::vector<sim::Obstacle>{});
...
TEST_CASE("ray obstacle: obstacle in front shortens measured distance") {
    ...
    const double d = ray::ray_distance_with_obstacles(pos, 0.0, offset, 0.0, wrap_obs(obs));
...
TEST_CASE("ray obstacle: diagonal ray intersects obstacle correctly") {
    ...
    const double d = ray::ray_distance_with_obstacles(pos, 45.0, offset, 0.0, wrap_obs(obs));
